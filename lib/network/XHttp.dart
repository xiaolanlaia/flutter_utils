...
class XHttp {
  ...
  String _getBaseUrl() => Constant.baseUrl;
  /// 通用全局单例，第一次使用时初始化。
  XHttp._internal() {
    ...
    dio.interceptors.add(
      InterceptorsWrapper(
        ...
        onResponse: (Response response, ResponseInterceptorHandler handler) {
          ...
          print("__response ${response?.data}");
          String code = (response?.data ?? {})['errorCode'].toString();
          String msg = (response?.data ?? {})['errorMsg'] ?? response.statusMessage;
          ...
          response.data = Result(parseData(response.data), isSuccess, response.statusCode!, msg, headers: response.headers);
  ...
  void _handleRequest(RequestOptions options, handler) {
    Toast.hide();
    EasyLoading.show(status: loadMsg);
    Map logData = {
  ...
  /// get 请求
  Future<ApiResponseEntity<T>> get<T>(String url, [Map<String, dynamic>? params, resultDialogConfig, bool isCancelWhiteList = false]) async {
    ...
    try {
      ApiResponseEntity<T> result;

      if (params != null) {
        response = await dio.get(url, queryParameters: params, cancelToken: requestToken);
        result = ApiResponseEntity.fromJson(jsonDecode(parseData((response.data as Result).data)));
        return result;
  ...
  /// post 请求
  Future<ApiResponseEntity<T>> post<T>(String url, [Map<String, dynamic>? data, resultDialogConfig, bool isCancelWhiteList = false]) async {
    // 可转为使用 request 代替，简化代码。
    Response response;
    CancelToken requestToken = CancelToken();
    if (dio.options.extra['cancelDuplicatedRequest'] != true || isCancelWhiteList) {
  ...
  /// put 请求
  Future<ApiResponseEntity<T>> put<T>(String url, [Map<String, dynamic>? data, resultDialogConfig, bool isCancelWhiteList = false]) async {
    // 可转为使用 request 代替，简化代码。
    Response response;
  ...
  /// patch 请求
  Future<ApiResponseEntity<T>> patch<T>(String url, [Map<String, dynamic>? data, resultDialogConfig, bool isCancelWhiteList = false]) async {
    // 可转为使用 request 代替，简化代码。
    Response response;
    CancelToken requestToken = {} as CancelToken ;
    if (dio.options.extra['cancelDuplicatedRequest'] != true || isCancelWhiteList) {
    ...
    try {
      ...
      ApiResponseEntity<T> result = ApiResponseEntity.fromJson(jsonDecode(parseData((response.data as Result).data)));
      return result;
    } catch (e) {
      _catchOthersError(e);
    } finally {
  ...
  /// delete 请求
  Future<ApiResponseEntity<T>> delete<T>(String url, [Map<String, dynamic>? data, resultDialogConfig, bool isCancelWhiteList = false]) async {
    // 可转为使用 request 代替，简化代码。
    Response response;
    CancelToken requestToken = {} as CancelToken ;
    if (dio.options.extra['cancelDuplicatedRequest'] != true || isCancelWhiteList) {
  ...
  /// request
  static Future<ApiResponseEntity<T>> request<T>(
      String url, {
  ...
  /// 下载文件
  Future<ApiResponseEntity<T>> downloadFile<T>(urlPath, savePath, [resultDialogConfig, bool isCancelWhiteList = false]) async {
    Response response;
    CancelToken requestToken = {} as CancelToken;
    if (dio.options.extra['cancelDuplicatedRequest'] != true || isCancelWhiteList) {
      if (isCancelWhiteList) {
  ...
  /// 设置当前 headers
  static XHttp setHeaders(Map<String,dynamic> headers) {
...
class Result {
  ...
  String errorMsg;
  var headers;
  Result(this.data, this.success, this.errorCode, this.errorMsg, {this.headers});
}
